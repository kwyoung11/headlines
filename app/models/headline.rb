class Headline < ActiveRecord::Base
	...
	def self.scrape(agency = :cnn, limit = 25, scraper_options)
		url, headline, other_stories = scraper_options[:base_urls][agency], scraper_options[:headlines][agency], scraper_options[:other_stories][agency]
		stories_arr, doc = [], Nokogiri::HTML(open(url))

  	stories_arr << "HEADLINER:\n " + doc.at_css(headline).text  # Appending individual headline to stories_arr[]
  	doc.css(other_stories).each_with_index do |link, i|  # Iterate over other news agency stories with an index
  		i >= limit ? break : i += 1	# Break iteration when @param limit is reached
  		stories_arr << "#{i}. " + link.at_css("a").text # Push link text onto stories_arr[]
  	end
  	return stories_arr
  end
  	...
  	scraper_options = {
  		...
  		:other_stories => {
  			:cnn => ".cnn_bulletbin li", :reuters => "h2", :chinadaily => "#main2 li, .headline-box h2, :nth-child(3) .wid300 h3", :bbc => "#top-story li, #second-story h2, #second_story li, 
				.secondary-story-header, #third-story li, #other-top-stories h3", :aljazeera => ".indexText-Font2 h2, div.indexText-Bold2.indexText-Font2, .h89-fix td div:first-child, .rightNewsArea, #ctl00_cphBody_ctl02_ctl01_DataList1_ctl00_Thumbnail1_Layout14 > div:nth-child(2), 
				...
				#ctl00_cphBody_ctl03_rptPosting_ctl02_Thumbnail1_Layout9 > div:nth-child(2), #ctl00_cphBody_ctl03_rptPosting_ctl03_Thumbnail1_Layout9 > div:nth-child(2), .skyscLines, .skyscBullet"
			}
  	}
  end
